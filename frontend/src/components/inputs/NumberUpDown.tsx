...
const NumberUpDown = ({ label, params, callBack, unit, label_unit }: { label?: string; params: string; callBack: CallableFunction; unit?: string; label_unit?: string }) => {
	...
	return (
		<div>
			...
			<div className={`num_updowun_container ${count !== 0 ? "active" : ""}`}>
				<button onClick={() => onClickCount(false)} className={`number_down ${count !== 0 ? "active" : ""}`}>
					<img src={IMGDown} alt="number down" />
				</button>
				<div className={`num_updown_unitbox fs_14 ${count !== 0 ? "active" : ""}`}>
					<div className="fs_14">{count}</div>
					<div className="fs_14">{unit}</div>
				</div>
				<button onClick={() => onClickCount(true)} className={`number_up ${count !== 0 ? "active" : ""}`}>
